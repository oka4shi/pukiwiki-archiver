...
- `/?記事名` のパスは `/記事名/index.html` に置き換えられる。サブページの場合はサブディレクトリとして扱われる。
- 例: `/?すごい記事` -> `/すごい記事/index.html`
- 例: `/?親記事/サブページ` -> `/親記事/サブページ/index.html`
...
- 編集: `/?cmd=edit&page=記事名` -> `/記事名/edit.html`
- 凍結: `/?cmd=freeze&page=記事名` -> `/記事名/freeze.html`
- 差分: `/?cmd=diff&page=記事名` -> `/記事名/diff.html`
- 添付: `/?plugin=attach&pcmd=upload&page=記事名` -> `/記事名/attach.html`

#### 各記事のBacklinksページ

各ページのヘッダーにある最後のリンク(`div#header > h1.title > a:last-child`)の先のページが取得される。保存されるURLは、以下のルールに従って変換される。

- `/?plugin=related&page=記事名` -> `/記事名/backlinks.html`
...
- `/?plugin=attach&pcmd=open&file=ファイル名&refer=記事名` -> `/記事名/_attachments/0/ファイル名`
- `/?plugin=attach&pcmd=open&file=ファイル名&refer=記事名&age=世代` -> `/記事名/_attachments/世代/ファイル名`
...
- `/?plugin=attach&pcmd=info&file=ファイル名&refer=記事名` -> `/記事名/_attachments/_info/0/ファイル名/index.html`
- `/?plugin=attach&pcmd=info&file=ファイル名&refer=記事名&age=世代` -> `/記事名/_attachments/_info/世代/ファイル名/index.html`
